...
/**
 * A command-line client. Uses jLine for console based input.
 * The command descriptions are specified in a XML descriptor compliant to a 
 * <a href="./commands.xsd">Command Specification Schema</a> . 
 * See <a href="./paradox-commands.xml">comamnds.xml</a> for an example.
 * <br>
 * Provides help for registered commands.
 * <br>
 * 
...
public abstract class AbstractCommandLineClient implements Runnable, ErrorHandler {
	...
	protected AbstractCommandLineClient() throws Exception {
		_output        = System.out;
		_consoleWriter = new PrintWriter(_output);
		_console       = new ConsoleReader(System.in, _consoleWriter);
		ConsoleReaderInputStream.setIn(_console);
	}
	/**
	 * Initializes a client with the given command specifications and given file name to store history.
	 * 
	 * @param commandSpec is the resource name of XML descriptor compliant to
	 * <a href="./commands.xsd">Command Specification Schema</a>
	 * @param prompt a command prompt
	 * @param historyFile file to record command history. null for no history. 
	 */
	protected void init(InputStream commandSpec, File historyFile) throws Exception {
		readCommandSpecifications(commandSpec);

		if (historyFile != null) {
			if (!historyFile.exists()) {
				historyFile.createNewFile();
			}
			_console.setHistory(new History (historyFile));
		}
	}
	 ...
	 * @param cmd moniker of the command
	 * @param line the entire command line
	 * @param options the parsed options
	 * @param args the parsed arguments
	 * @throws Exception raise exception from execution. This client will print it on console.
	 */
	 ...
	 * @param cmd moniker of a given command. If null prints the available commands
	 */
	public void help(String cmd) {
		...
		if (meta == null) {
			print("*** " + cmd + " is not a known command. Type help for available commands");
			return;
		}
	 ...
	 * @param margin1 the column index where first column begins
	 * @param col1 the content for first column
	 * @param margin2 the column index where second column begins
	 * @param col2 the content for second column
	 * @param maxWidth maximum width of a line
	 */
	void printIndented(int margin1, String col1, int margin2, String col2, int maxWidth) {
		int col = 0;
		for (; col < margin1; col++) printGap();
		print(col1);
		col += col1.length();
		for (; col < margin2; col++) printGap();
	...
	String readConsole() {
		try {
			return _console.readLine(_prompt);
	...
	/**
	 * Reads the contents of the given input as an XML document for command metadata.
	 * @param in
	 * @throws Exception
	 */
	...
	private static enum ParseState {READ_OPTION, READ_OPTION_VALUE, READ_KEY, READ_ARGS};
	
	/**
	 * Parses the command line according to command metadata
	 * and invokes execution of the command to the concrete subclass
	 * with the parsed option values and arguments
	 * 
	...
	public static class CommandMetaData {
		...
		boolean validate(Map<String,String> optionValues, List<String> args) throws ParseException {
			...
			if (args.size() < argMin) {
				throw new ParseException("Insufficient arguments. Minimum " + argMin + " required, " 
						+ args.size() + " supplied", 0);
			}
			return true;
		}
		
		public String getUsage() {
			if (!requiresParse) return usage;
			StringBuffer buf = new StringBuffer();
			buf.append(key).append(" ");
			for (CommandOption option : _options) {
				buf.append(option); buf.append(" ");
			}
			for (CommandArg arg : _args) {
				buf.append(arg); buf.append(" ");
			}
			return buf.toString();
		}
	...
	public static class CommandOption {
		...
		public String toString() {
			...
			if (requiresValue) buf.append(" <").append(name).append('>');
			buf.append(optional ? ')' : ']');
